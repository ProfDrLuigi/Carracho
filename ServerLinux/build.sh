...
CFLAGS="$(pkg-config --cflags openssl json-c sqlite3)"
LIBS="$(pkg-config --libs openssl json-c sqlite3)"
...
"$CC" -std=c11 -O2 -Wall -Wextra -Wpedantic -Wmisleading-indentation $EXTRA_CFLAGS -Wno-deprecated-declarations -pthread $CFLAGS \
  ServerLinux/main.c ServerLinux/server_runtime.c ServerLinux/server_state.c ServerLinux/sqlite_state.c ServerLinux/carracho_protocol.c ServerLinux/file_metadata.c ServerLinux/file_search_index.c ServerLinux/news_store.c ServerLinux/flat_news_store.c ServerLinux/media_store.c \
