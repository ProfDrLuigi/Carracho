...
enum LegacyMD5 {
    ...
    static func digest(_ input: Data) -> Data {
        ...
        for chunkOffset in stride(from: 0, to: message.count, by: 64) {
            ...
            for i in 0..<16 {
                let base = chunkOffset + i * 4
                words[i] = UInt32(message[base]) |
            ...
            for i in 0..<64 {
                ...
                b = b &+ rotateLeft(sum, by: shifts[i])
                a = next
            }
            a0 = a0 &+ a
            b0 = b0 &+ b
    ...
    static func hexDigestASCII(_ input: Data) -> Data {
        Data(digest(input).flatMap { byte -> [UInt8] in
